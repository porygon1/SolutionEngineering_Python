...
def get_album_artwork(track_info: Dict, spotify_client=None) -> Optional[str]:
    ...
    if spotify_client and track_info.get('id'):
        try:
            track = spotify_client.track(track_info['id'])
...
def render_bottom_player(track, artist_mapping, spotify_client):
    """Render a Spotify-like bottom music player"""
    
    if track is None:
        return
    
    # Create the bottom player container
    st.markdown("---")
    st.markdown("### 🎵 Now Playing")
    
    # Player container with fixed layout
...
def render_progress_bar(track):
    ...
    with col1:
        ...
        st.markdown(f"<div style='color: #b3b3b3; font-size: 0.8rem;'>{current_time}</div>", 
                   unsafe_allow_html=True)
    ...
    with col3:
        ...
        st.markdown(f"<div style='color: #b3b3b3; font-size: 0.8rem;'>{total_time}</div>", 
                   unsafe_allow_html=True)

def render_player_volume_controls(track, spotify_client):
    """Render volume and additional controls"""
    
    # Use HTML layout instead of nested columns to avoid Streamlit restrictions
    controls_html = f"""
    <div style="display: flex; gap: 1rem; align-items: center; justify-content: space-between; padding: 0.5rem 0;">
    ...
    # Render controls in separate sections to avoid nesting
    ...
    if is_liked:
        if st.button("💚 Remove from Liked", key="unlike_track", help="Remove from Liked"):
            st.session_state[f'liked_{track.get("name", "")}'] = False
            st.rerun()
    else:
        if st.button("🤍 Add to Liked", key="like_track", help="Add to Liked"):
    ...
    volume = st.slider(
        ...
        min_value=0, 
        max_value=100,
        value=st.session_state.get('volume', 70),
        key="volume_slider",
        label_visibility="collapsed"
    )
    
    if volume != st.session_state.get('volume', 70):
        st.session_state.volume = volume
    
    # Additional actions
    if st.button("⋯ More Options", key="more_options", help="More Options"):
        render_track_options(track, spotify_client)
...
def render_track_options(track, spotify_client):
    ...
    with st.expander("🎵 Track Options", expanded=True):
        ...
        if st.button("🎯 Get Similar", use_container_width=True, key="get_similar"):
            # Set this track for recommendations
            track_idx = track.name if hasattr(track, 'name') else st.session_state.selected_track_idx
            st.session_state.selected_track_idx = track_idx
            st.success("Track selected for recommendations!")
            st.rerun()
